...
class CGame:
	...
	def load_layout(self):
		...
		self.scoreDict = self.aiPath.getNewScoreDict()
		num_of_game_elements = 7 
		
		keys = self.data.keys()
		for key in keys:
			self.dbuffer.setPixel(int(key), self.colorCoins , 1, self.intensityCoins)

		#0,1,2 ghosts, 3,4,5 special beans, 6 pacman
		temp_list = random.sample(keys,7)	
			
		key = int(temp_list[6])
                self.dbuffer.setPixel(key, self.colorPacMan , 1, self.intensityPacMan)
                self.posPacMan = key
                self.indexPacMan = str(self.posPacMan)
		...
		if  self.secondPlayerActive == True:
		    self.dbuffer.setPixel(key, self.colorGhost , 1, self.intensityGhost)
		    self.posGhost = key
		    self.indexGhost = str(self.posGhost)
		else:
		#if second joystick is inactive use this
		    self.dbuffer.setPixel(key, self.colorAIGhost2 , 1, self.intensityAIGhost)
		    self.posAIGhost2 = key
		    self.indexAIGhost = str(self.posAIGhost2)
		
		key = int(temp_list[1])
		self.dbuffer.setPixel(key, self.colorAIGhost1 , 1, self.intensityAIGhost)
		self.posAIGhost1 = key
		self.indexAIGhost = str(self.posAIGhost1)

		key = int(temp_list[2])	
		self.dbuffer.setPixel(key, self.colorAIGhost3 , 1, self.intensityAIGhost)
		self.posAIGhost3 = key
		self.indexAIGhost = str(self.posAIGhost3)

		for i in xrange(3,6):
			key = int(temp_list[i])
			self.dbuffer.setPixel(key, self.colorSpecialBean, 1, self.intensitySpecialBean)
			self.posSpecialBeans.append(key)
